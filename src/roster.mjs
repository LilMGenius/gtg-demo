...
// 세이브 한 번이 12땀이라 초반에는 최하위권만 산다. 명단이 바뀌면 자릿수가 통째로 움직이므로
// 여기에 실제 값을 옮겨 적지 않는다. 지금 값은 pull 게이트가 매 실행에 출력한다.
const COST_BASE = 27;
...
// 이적시장 한 장 값. 무작위 한 장이 이름을 찍는 것보다 비싸면 뽑을 이유가 사라지므로,
// 지금 살 수 있는 가장 싼 이름보다 낮게 둔다. 견줄 대상은 명단 맨 아래가 아니다.
// 그 자리는 첫 진입이 모두에게 주는 시작 키퍼라 아무도 그 이름을 사지 않는다.
// 그 위 한 칸이 실제 대안이고, pull 게이트가 매 실행에 그 이름과 값을 출력한다.
