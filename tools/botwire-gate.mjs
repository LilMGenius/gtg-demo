...
try {
  ...
  const watch = async (ms, clickPad, until, armOn) => {
  ...
  const bot = await watch(BOT_MS, false, null, true);
  ...
  const hand = await watch(HAND_MS, true, (b, n) => sum(n.rap) > sum(b.rap), false);
