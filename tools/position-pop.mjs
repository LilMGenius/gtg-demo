...
// 옛 두 정책은 product-pop이 이름으로 찾으므로 비교 표본으로 유지한다.
export const MODES = ['hand-centre', 'hand-react', 'hand-bait-react', 'bot', 'hand-follow', 'hand-bait'];
...
export function positionInput(keeper, shot, rng, mode = 'hand-react', pRead = P_READ) {
  ...
  if (mode === 'hand-react' || mode === 'hand-bait-react') {
    // U1 미끼의 0.3 편위와 600ms 선행 이동은 그대로 두고, 인간은 접촉 250ms 뒤 90% 적중으로 따른다.
    const pre = mode === 'hand-react'
      ? [{ ms: -1000, x: 0 }, { ms: 0, x: 0 }]
      : [{ ms: -1000, x: 0.3 }, { ms: -600, x: 0.3 }, { ms: 0, x: clampX(0.3-speed*0.6) }];
    const aimed = chain.aimAt(keeper, shot, pre);
    const x = pre.at(-1).x;
    const target = rng() < pRead ? aimed.aimX : x - (aimed.aimX-x);
    return fromTrace(chain.reactTrace(keeper, aimed, pre, 250, target), false);
  }
  if (mode === 'hand-bait') {
