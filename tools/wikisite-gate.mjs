...
try {
  ...
  execFileSync(process.execPath, [CLI, 'build', SOURCE, '--out', join(temp, 'fresh'), '--base', BASE], { cwd: ROOT, encoding: 'utf8', windowsHide: true, timeout: 90000 });
  const tracked = hashes(SITE);
  ...
  const entities = JSON.parse(readFileSync(join(ROOT, 'web/wiki/dist/entities.json'), 'utf8'));
  ...
  check('wikisite:the-site-links-back-to-the-game', /href="\.\.\/\.\.\/index\.html"/.test(game), { page: 'game.html' });
