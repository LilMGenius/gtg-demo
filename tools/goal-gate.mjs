...
//   1. 골 입구 네 꼭짓점이 전부 화면 안. 축 3이 이 사각형으로 공을 판정하므로 입구가 잘리면 축 3이 무의미해진다.
//      이 축의 대상은 __goalFrame이 재는 앞 골대 입구 하나뿐이다(stage-probe.mjs goalFraming).
//      골대의 다른 부위, 특히 뒷틀과 뒷기둥이 화면 밖으로 나가는 것은 이 축과 무관하고 결함도 아니다.
//      실패 라벨 GOALCUT은 "골대가 잘렸다"가 아니라 "입구를 판정할 수 없다"로 읽어라.
