...
const PANELS = [
  ...
  ["gym", async (p) => {
    await p.evaluate(() => { ["shop", "roster", "gram", "me", "wiki"].forEach((w) => window["__" + w](false)); window.__date(); });
    await p.waitForFunction(() => {
      const open = ["gym", "roster", "gram", "me", "shop", "wiki", "date"].some((id) => !document.getElementById(id).hidden);
