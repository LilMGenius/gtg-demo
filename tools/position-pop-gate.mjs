...
import { resolve as legacyResolve } from '../src/chain.mjs';

// 이백 시드의 다섯 슛에서 네 정책과 열다섯 스탯을 교차해 난수 분기와 자취 배선을 검사한다.
const SEEDS = 200;
let pairs = 0, membershipDrift = 0, noTrace = 0, speedErrors = 0, replayDrift = 0, changed = 0, moving = 0, changedMembership = 0, legacyNoTrace = 0, speedControl = 0;
...
for (let seed = 1; seed <= SEEDS; seed++) {
  ...
  for (const shot of buildSet(makeRng(seed))) for (const mode of MODES) {
    ...
    const legacy = legacyResolve({ keeper: { ...keeper }, shot, rng: makeRng(seed) });
    legacyNoTrace += Number(!Array.isArray(legacy.input?.trace));
...
const checks = {
  'trace-and-speed': noTrace === 0 && legacyNoTrace > 0 && speedErrors === 0 && speedControl > 0 && moving > 0,
...
console.log(JSON.stringify({ pairs, membershipDrift, changedMembership, noTrace, legacyNoTrace, speedErrors, speedControl, replayDrift, changed, moving }));
