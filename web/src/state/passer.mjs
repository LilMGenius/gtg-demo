...
/* 얼굴표 판번호. 위 표에서 자리가 한 번이라도 움직이면 이 수가 오르고 아래 이동표에 그 줄이 선다.
   ...
   수가 여러 개인 것은 표가 여러 번 움직여서가 아니다. 그 한 번을 어느 지도에 적용했는지가 판마다 다르다.
...
/* 판 3은 글이다. 글은 (도시, 번호)를 키로 안 쓰고 기록 안에 칸으로 들고 있어서 앞의 두 걸음이 안 건드렸다.
   피드의 선팔 버튼이 그 칸을 그대로 읽으므로, 안 옮기면 옛 글 아래 버튼이 지금 그 자리에 앉은 딴 사람을 건다. */
export const FACES_V_POSTS = 3;
// 새로 쓰는 저장에 찍는 수. 지도를 하나 더 옮기는 날 여기가 그 새 수를 가리킨다.
export const FACES_V = FACES_V_POSTS;
