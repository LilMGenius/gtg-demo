...
// 장비도 같이 나간다. 산 장갑이 탭을 닫을 때 벗겨지면 상점에서 쓴 골드가 사라진다.
...
export function save(squad, pick, auto, fans, points, wallet, posts, record, gear, bot, buff, rapport, tickets, social, kickers, eleven, onboard, _retiredPref, coach = false) {
  try {
    ...
    // 옛 호출 순서는 받되 고정 방향 선호는 저장하지 않는다.
    localStorage.setItem(KEY(), JSON.stringify({ squad, pick: i, keeper: squad[i], auto, coach, fans, points, wallet, posts: Array.isArray(posts) ? posts : [], record: record || {}, gear: gear || {}, bot: bot || {}, buff: buff || {}, rapport: rapport || {}, faces: FACES_V, tickets: Number(tickets) || 0, social: social || {}, kickers: Array.isArray(kickers) ? kickers : [], eleven: Array.isArray(eleven) ? eleven : [], onboard: Number(onboard) || 0, at: Date.now() }));
  } catch {
