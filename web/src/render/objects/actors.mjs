...
// 개최지별 의상 팔레트와 이름은 상태 풀의 식별자를 그대로 쓴다. 몸은 승인된 행인 기하가 소유한다.
...
// 승인된 행인의 흔들림을 이동 거리에 묶어 멈춘 몸이 제자리에서 걷지 않게 한다.
export function poseWalker(rig, distance, {heading = 0} = {}) {
  rig.root.rotation.y = heading;
  // 승인된 보행 흔들림의 주기 6과 진폭 0.06을 그대로 유지한다.
  rig.root.rotation.z = Math.sin(distance * 6 + rig.root.userData.phase) * 0.06;
}
