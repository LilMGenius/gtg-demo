...
/* 세이브 포즈의 예비와 잔여. 사건마다 몸이 다르게 무너지는데 되감는 깊이와 떠는 주기는 상수 한 쌍이라,
   ...
   규칙은 하나다. 대기 자세에서 멀리 무너진 몸일수록 깊게 되감고 길게 떤다. 순서는 손대중이 아니라
   각 포즈가 대기 자세에서 떨어진 관절 각 거리이고(실측 L2, 라디안) 표는 그 순서로 적혀 있다.
   그 거리가 움직이거나 두 수의 순서가 어긋나면 pose 게이트의 beat 축이 빨개진다.
...
export const POSE_BEAT = {
  ...
  swatR: { ant: 1.10, per: 0.50 },
  // 1.850. 공을 가슴에 안으면 진동을 공이 먹는다. 되감을 자리도 팔 안쪽뿐이다.
  clutch: { ant: 1.14, per: 0.56 },
  // 1.944. 잡자마자 달려 나간다. 걸음이 살아 있어 앞의 둘보다는 길게 떤다.
  dribble: { ant: 1.18, per: 0.62 },
  // 2.261. 한 손으로 걷어 올린다. 팔 하나가 크게 돌아 되감김이 그만큼 깊어진다.
  snatch: { ant: 1.20, per: 0.70 },
  // 2.719. 손끝까지 뻗는다. 표의 가운데 줄이고 고치기 전의 한 쌍이 여기 서 있다.
  reachR: { ant: 1.22, per: 0.84 },
  // 2.871. 제껴져 휘청인다. 다리가 살아 있어 진동이 더 오래 남는다.
  stumble: { ant: 1.24, per: 0.90 },
  // 2.958. 넘어간 공을 올려다본다. 상체가 통째로 젖혀져 천천히 흔들린다.
  skyward: { ant: 1.26, per: 0.94 },
  // 2.962. 공을 안은 채 끌려 들어간다. 팔이 묶여 못 짚으니 몸통이 그만큼 더 떤다.
  hugfall: { ant: 1.28, per: 0.96 },
  // 3.251. 몸을 가로질러 세게 민다. 어깨가 따라 돌아 되감김이 깊다.
  shoveR: { ant: 1.32, per: 1.02 },
  // 3.429. 두 손이 머리로 올라가고 허리가 접힌다. 몸 전체가 한 번에 무너진다.
  despair: { ant: 1.34, per: 1.06 },
