...
//
// 이 판의 눈금은 1 단위 = 1 m이고 골대도 실물 GOAL 규격으로 서 있다. 선만 축소판을 쓰면
// 한 화면이 두 경기장을 그린 것이 된다. 그래서 전부 실제 규격이다.
export const BOX_W = PITCH_MARKS.values.boxW;
export const BOX_D = PITCH_MARKS.values.boxD;
export const AREA_W = PITCH_MARKS.values.areaW;
export const AREA_D = PITCH_MARKS.values.areaD;
export const SPOT_Z = PITCH_MARKS.values.spot;
export const ARC_R = PITCH_MARKS.values.arcR;
// 골라인은 화면 밖까지 이어져야 땅이 끊긴 것으로 안 읽힌다. 60은 페널티 에어리어보다 넉넉하다.
export const GOAL_LINE_W = 60;
// 선 굵기. PITCH_MARKS의 최대 선 굵기를 그대로 쓰면 20 m 밖에서 한 화소 아래로 내려가 아크가 화면에서 사라진다.
// 이 게임은 골대 뒤에서 땅을 스치듯 보므로 먼 선일수록 두껍게 그어야 같은 굵기로 읽힌다.
export const NEAR_W = PITCH_MARKS.values.lineMax;
export const FAR_W = 0.34;

// 아크가 박스 밖으로 나온 몫. PITCH_MARKS의 스팟에서 박스 앞선까지 거리이므로 그 바깥만 반달로 남는다.
