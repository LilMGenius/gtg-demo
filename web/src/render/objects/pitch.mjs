...
export function buildPitch(scene) {
  ...
  //
  // 틀은 그물 상자의 뒷면 그 자리에 선다. 그물 폭 R_HALF_W, 그물 높이 R_H, 그물 뒷면 -NET_D.
  // 여기서 벗어난 값을 쓰면 실은 상자인데 뼈대만 다른 크기라, 뒤로 갈수록 솟거나 좁아지는
  // 실물에 없는 모양이 된다. 뒷기둥이 화면 밖으로 나가는 것은 결함이 아니다.
  // 카메라가 골대 뒤에 있으니 실제 골대도 뒷기둥은 시야 밖에 있다.
  const BACK_HW = R_HALF_W;
  // 뒷 가로대는 크로스바와 같은 높이지만 카메라(y 3.3)보다 낮아 가까운 쪽이 화면에서 더 내려온다.
  // 크로스바 아래에 나란한 띠 하나로 읽혀 그것이 상자의 천장이 된다.
  const BACK_H = R_H;
  ...
  // 뒷 상단을 가로대로 잇지 않는다. 실측: 그 자리(0, R_H, -NET_D)는 화면 y 364이고
  // 원거리 비행 공(0, 0.14, 11)은 365다. 카메라가 골대 뒤 높은 곳에서 내려다보므로
  // 뒷틀 상단과 멀리 있는 공은 화면에서 같은 줄에 온다. 두께는 뒷틀 17px, 공 14px이라
  // 굵은 쪽이 얇은 쪽을 완전히 삼켰다. 45프레임 동안 공이 화면에서 사라진 원인이 이것이다.
  // 낡은 동네 골대의 뒷 상단은 원래 쇠가 아니라 그물 자신이다. 그물은 투명이라 공을 가리지 않는다.
  // 대신 그물 밑단이 묶이는 바닥 가로대를 놓는다. 실물 골대가 반드시 가지고 있는 물건이고,
  // 화면에서는 y 748로 프레임 아래라 무엇도 가리지 않는다.
