...
const TOUCHED = new Set(['contact']);
  ...
  // 키커는 10.5m 뒤에 서 있어 화각을 넓게 잡으면 80px짜리 점이 되어 흙바닥에 묻힌다.
  // 그래서 뒤로 빼고 화각을 좁히는 망원 압축을 쓴다. 거리로 잃는 크기보다
  // 좁힌 화각으로 얻는 크기가 크고, 먼 키커와 가까운 크로스바의 화면상 간격도 같이 벌어진다.
  // 높이는 3.3에서 키커 발끝이 골대 상단 프레임에 겹쳤고 4.4는 크로스바가 키퍼 턱을 잘랐다.
  ...
  function frame() {
    ...
    if (cue) {
      ...
      if (t < runup) {
        ...
        if (p >= 1) {
          ...
          // 약한 구는 -0.92, 실이 거의 안 늘어나 그 자리에서 눕는다.
          // 센 구는 -0.30까지 되밀려 나온다. 골라인(0)은 넘지 않으니 들어간 것으로 계속 읽힌다.
          // -1.15는 약한 구의 착탄 지점 -1.121보다 깊어 그물이 공을 0.03 빨아들이고 있었다.
          // 막은 공은 반대로 골라인 앞으로 나온다. 0.55는 키커 쪽이라 다음 구의 시작점으로 읽힌다.
          const restZ = result.conceded ? lerp(-0.92, -0.30, cue.force) : 0.55;
          // 되밀림을 0.22초 이징으로 끊으면 그 뒤 낙하 0.94m 동안 깊이가 완전히 굳는다.
          // 실측: bz -0.654가 13프레임 연속 소수점 셋째 자리까지 같았고, 공이 줄에 매달려 내려왔다.
          // 되밀림과 낙하는 한 번의 튕김이 낳은 한 몸이라 두 개의 직선으로 끊으면 안 된다.
          // 그물에서 나온 속도가 실과 공기에 끌려 잦아드는 것이니 모양은 지수감쇠다.
          // 3.2/초면 착지(낙하 약 0.56초)까지 83%가 지나가고 남은 몫이 바닥에서 굴러 빠진다.
          const K_SETTLE = 3.2;
          const decay = 1 - Math.exp(-K_SETTLE * u);
          ball.position.z = s0.z + (restZ - s0.z) * decay;
          // 옆으로 눕는 것도 같은 튕김의 성분이다. 같은 감쇠를 태워야 하나의 곡선으로 읽힌다.
          ball.position.x = s0.x + s0.x * (0.76 - 1) * decay;
    ...
    if (tail) {
      ...
      // 준비 자세에서 최종 자세로 가는 선 위에서 회차마다 조금 더 가거나 덜 간다.
      // 선 위를 움직이므로 다른 사건의 자세로 넘어가지 않는다. 신원은 kpId가 이미 들고 있다.
      kp = pushPose(POSES.ready, kp, 1 + (tail.vary.c - 0.5) * 0.20);
      ...
      switch (tail.kind) {
        ...
        case 'charge':
          ...
          // 이 사건만 제 편차가 없었다. 나머지 열넷은 각자 tail.vary로 종점을 흔드는데
          // 돌진은 공유 포즈 블렌드에서 흘러온 것만 받아, 두 회차가 그 값만 비슷하면 그림이 포개졌다.
          // 열네 회차 아흔한 쌍에서 가장 닮은 쌍이 0.0035까지 내려갔고 중앙값은 0.0834이었다.
          // 세 축을 각각 다른 성분에 걸어 회차가 포개지려면 셋이 동시에 맞아야 하게 만든다.
          // 성분이 0.5면 아래 셋은 예전 상수와 같은 값이 되므로 대조군과 평균 그림은 안 움직인다.
          // z=6.5까지 보내면 키퍼가 골대 그물 너머 원경에 파묻히고 공이 몇 픽셀로 줄어든다.
          // 나갔다는 사실은 페널티 박스를 벗어나는 것으로 이미 읽힌다. 근경에 세운다.
          // 얼마나 멀리 나가 서는지는 매번 다르다. 다만 폭 0.8은 멈춘 프레임의 공을 z 3.0에서 3.2
          // 사이로 자주 보냈고, 그 구간은 골대 뒤틀에 가려 세 번 중 두 번 공이 안 보였다.
          // 폭을 0.3으로 좁혀 그 띠를 안 밟는다. 회차 간 편차는 좌우 쏠림과 드리블 리듬이 이미 지고 있다.
          keeper.position.z = lerp(KEEPER_Z, CHARGE_Z + (tail.vary.a - 0.5) * 0.3, e);
          // 카메라가 골대 뒤에 있으므로 키퍼보다 먼 자리에 둔 공은 무조건 등에 가려진다.
          // 드리블하는 공은 카메라 쪽 발 옆으로 온다. 그래야 몸과 안 겹치고 발 옆으로 읽힌다.
          // 나가면서 한쪽으로 쏠린다. 꼬리 동안 x를 쓰는 곳이 없어(위 갱신은 !tail 안이다)
          // 이 값은 꼬리 내내 tail.kx로 굳어 있다. 그래서 tail.kx에서 절대값으로 다시 세우면
          // 성분이 0.5일 때 지금과 같은 자리가 되고 프레임마다 쌓이지도 않는다.
          // 높이나 리듬과 달리 이 축은 어느 프레임에서 찍어도 벌어진다. 사인이 0 근처인 순간에는
          // 높이 편차가 아무것도 못 벌리고, 그 프레임에서 두 회차가 다시 포개졌다.
          keeper.position.x = tail.kx + (tail.vary.b - 0.5) * 1.1 * e;
          // 카메라가 골대 뒤에 있으므로 키퍼보다 먼 자리에 둔 공은 무조건 등에 가려진다.
          // 드리블하는 공은 카메라 쪽 발 옆으로 온다. 그래야 몸과 안 겹치고 발 옆으로 읽힌다.
          // 발과의 거리 0.78은 안 건드린다. 몸 쪽으로 좁히면 공이 등에 가려질 수 있는데
          // 가림을 재는 자는 선방과 안고 넘어짐과 장갑과 쓰러짐 넷만 보고 돌진은 안 본다.
          // 키퍼가 쏠리면 공도 같은 간격을 유지한 채 따라가므로 이 축은 가림을 안 건드린다.
          // 0.34만 앞에 두면 멀리 나간 공이 화면에서 뒷틀 상단 가로대 높이로 올라가 그 쇠 뒤에 선다.
          // 그 가로대의 주석은 골 안으로 들어온 공만 그 아래를 지난다고 적혀 있는데, 필드로 나간
          // 공은 그 위를 지난다. 카메라 쪽으로 더 내놓으면 투영이 내려와 쇠를 벗어난다.
          ball.position.set(
            keeper.position.x + 0.78,
            0.14 + Math.abs(Math.sin(u * (10 + tail.vary.c * 4))) * 0.42,
            keeper.position.z - 0.9
          );
          break;
        case 'beat':
      ...
      // 램프의 증가분만 더한다. 폭 0.13라디안은 7.4도라 사건이 다른 사건으로 넘어가지 않는다.
  ...
  window.__squashVis = () => {
    ...
    return { x: s.x, y: s.y, z: s.z, left: landSq, lie: lieNow,
      ...
      landFall: tail && tail.landed ? tail.landFall : -1,
