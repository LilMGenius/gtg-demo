...
export function createScene(canvas) {
  ...
  // 높이와 시선을 같이 올린 이유는 키커가 골대 상단 프레임에 겹쳐 반쯤 잘려 보였기 때문이다.
  // 카메라를 올리면 가까운 골대 상단은 화면에서 크게 내려가고 먼 키커는 조금만 내려간다.
  // 그 차이가 둘을 갈라놓는다. 3.3에서는 키커 발끝 346이 상단 프레임 안이었고,
  // 4.4는 갈라졌지만 크로스바가 키퍼 턱을 잘랐다. 4.0이 둘 다 비켜간 값이다.
  const CAM_BASE = new THREE.Vector3(0, 4.0, -6.2);
  const CAM_LOOK = new THREE.Vector3(0, 1.25, 4.5);
  let shakeAmp = 0;
  let shakeLeft = 0;
  let shakeSpan = 1;
