...
export function createScene(canvas) {
  ...
  const CAM_EV = {
    save: { pos: [0, -0.3, 0.6], look: [0, -0.08, 0], fov: -1, dur: 0.8 },
    // 쳐낸 것과 잡은 것이 같은 각도면 둘은 같은 그림이다. 잡은 것은 옆에서 붙어
    // 가슴에 안긴 공을 본다. 정면에서 조금 당기기만 하면 선방과 구별이 안 된다.
    catch: { pos: [-0.55, -0.12, 0.5], look: [-0.22, -0.02, 0], fov: -2, dur: 0.75 },
    // 골라인이 화면을 가로지르게 낮춘다. 위에서 보면 넘었는지 앞인지 판정이 안 선다.
    carriedIn: { pos: [0, -1.15, 0.7], look: [0, -0.42, 0.5], fov: -1, dur: 1.1 },
    // 벗겨진 장갑 쪽으로 붙는다. 장갑이 팔 옆에 그대로 있는 것으로 읽히면 인과가 끊긴다.
    gloveGone: { pos: [0.7, -0.42, 0.6], look: [0.28, -0.16, 0], fov: -2, dur: 0.95 },
    // 깔린 키퍼를 크게 잡는다. 광각에서는 누가 쓰러졌는지가 몇 픽셀 차이다.
    downed: { pos: [0, -0.85, 0.75], look: [0, -0.52, 0.25], fov: -1, dur: 1.05 },
    // 키퍼가 골라인을 떠나 앞으로 나간다. 렌즈가 따라붙어야 돌진으로 읽힌다.
    charge: { pos: [0, -0.5, 0.9], look: [0, -0.12, 0.45], fov: 0, dur: 0.9 },
    spill: { pos: [0, -0.2, 0.4], look: [0, -0.06, 0], fov: 0, dur: 0.7 }
  };
  let camEv = null;
  let camEvLeft = 0;
  let camEvSpan = 1;
